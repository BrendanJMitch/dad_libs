 ...
 * level 23 implementation was not used because its implementation is more
 * complex.
 *
 * This copy has the following modifications applied:
 *     - GraphicsOperations is not implemented since it is a hidden interface
 *     - Copies of EmptyArray, ArrayUtils, and GrowingArrayUtils are used
 *     because they are internal classes
 *     - Increase visibility of
 *     {@link #sendSpanChanged(Object, int, int, int, int)} to allow subclasses
...
public class ModifiedSpannableStringBuilder implements CharSequence, GetChars, Spannable, Editable, Appendable {
    ...
    private void change(int start, int end, CharSequence cs, int csStart, int csEnd) {
        ...
        if (replacedLength > 0) { // no need for span fixup on pure insertion
            ...
            int i = 0;
            while (i < mSpanCount) {
                if ((mSpanFlags[i] & SPAN_EXCLUSIVE_EXCLUSIVE) ==
    ...
    private static boolean hasNonExclusiveExclusiveSpanAt(CharSequence text, int offset) {
        if (text instanceof Spanned) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (flags != SPAN_EXCLUSIVE_EXCLUSIVE) return true;
            }
        }
        return false;
    }
